...
pub fn cut(origin_text: &String) -> Vec<String> {
    ...
    .map(|s| {
        paren_repair.replace_all(&s.to_string(), r"$1)")
    })
    // Repair quotes with sentence enders.
    .map(|s| {
        quote_one_repair.replace_all(&s.to_string(), r#"'$p""#)
    })
    .map(|s| {
        quote_two_repair.replace_all(&s.to_string(), r#"'$p”"#)
    })
    .map(|s| {
        quote_three_repair.replace_all(&s.to_string(), r#"$p”"#)
    })
    .map(|s| {
        quote_four_repair.replace_all(&s.to_string(), r#"'""#)
    })
    .map(|s| {
        quote_five_repair.replace_all(&s.to_string(), r#"$p'"#)
    })
    .map(|s| {
        quote_six_repair.replace_all(&s.to_string(), r#"$p""#)
    })
    .filter_map(|s| {
        if s.len() > 1 {
            Some(s.to_string())
